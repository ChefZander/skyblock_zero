...
local radiation_check = function(pos)
    local nodes =
        core.find_nodes_in_area(pos - vector.new(5, 5, 5), pos + vector.new(5, 5, 5), { 'group:radioactive' }, true)
    local rad = 0
...
function sbz_api.register_plant(name, defs)
    ...
    for i = 1, defs.stages - 1 do
        ...
        minetest.register_node('sbz_bio:' .. name .. '_' .. i, {
            ...
            selection_box = {
                type = 'fixed',
                fixed = { -defs.width, -0.5, -defs.width, defs.width, height, defs.width },
            },
            paramtype = 'light',
    ...
    minetest.register_node('sbz_bio:' .. name .. '_' .. defs.stages, {
        ...
        selection_box = {
            type = 'fixed',
            fixed = { -defs.width, -0.5, -defs.width, defs.width, defs.height_max, defs.width },
        },
        paramtype = 'light',
        sunlight_propagates = true,
        paramtype2 = 'color',
        ...
        growth_tick = defs.use_co2_in_final_stage and function()
            return true
        end,
        grow = defs.use_co2_in_final_stage and function()
            return true
        end,
...
minetest.register_node('sbz_bio:fiberweed', {
    ...
    selection_box = {
        type = 'fixed',
        fixed = { { -0.5, -0.5, -0.5, 0.5, 0.5, 0.5 }, { -0.25, 0.5, -0.25, 0.25, 1.25, 0.25 } },
    },
    paramtype2 = 'leveled',
    place_param2 = 8,
