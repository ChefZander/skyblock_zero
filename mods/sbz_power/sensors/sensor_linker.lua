...
local function make_link(meta, pos, placer)
    ...
    if linked == '' then return core.chat_send_player(placer_name, "Tool isn't linked to any machine.") end
    ...
    if ndef.add_link then
        ...
        for lname, lpos in pairs(links) do
            for lname_2, lpos_2 in ipairs(lpos) do
                if vector.equals(pos, lpos_2) then names[#names + 1] = { lname, lname_2 } end
            end
        end
        if #names >= 2 then
            local tables_to_fix = {}
...
minetest.register_on_player_receive_fields(function(player, formname, fields)
    ...
    if wield_item:get_name() ~= 'sbz_power:sensor_linker' then return true, 'sus' end
