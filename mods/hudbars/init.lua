    ...
    if sval ~= nil then
        if valid_values ~= nil then
            ...
            for i = 1, #valid_values do
                if sval == valid_values[i] then valid = true end
            end
            if not valid then
                minetest.log(
                    'error',
...
local function make_label(format_string, format_string_config, label, start_value, max_value)
    if hb.settings.hide_labels then return '' end
...
function hb.get_hudbar_position_index(identifier)
    if hb.settings.sorting[identifier] ~= nil then
        ...
        local i = 0
        while true do
            if hb.registered_slots[i] ~= true and hb.settings.sorting_reverse[i] == nil then return i end
            i = i + 1
        end
    end
end

function hb.register_hudbar(
    identifier,
    text_color,
    label,
    textures,
    ...
    if format_string == nil then format_string = NS '@1: @2/@3' end
    if format_string_config == nil then format_string_config = {} end
    if format_string_config.order == nil then format_string_config.order = { 'label', 'value', 'max_value' } end
    if format_string_config.format_value == nil then format_string_config.format_value = '%d' end
    if format_string_config.format_max_value == nil then format_string_config.format_max_value = '%d' end
    ...
    hudtable.add_all = function(player, hudtable, start_value, start_max, start_hidden)
        ...
        local main_error_text = '[hudbars] Bad initial values of HUD bar identifier “'
            .. tostring(identifier)
            .. '” for player '
            .. name
            .. '. '
...
function hb.change_hudbar(
    ...
    if
        new_value == nil
        and new_max_value == nil
        and new_icon == nil
        and new_bgicon == nil
        and new_bar == nil
        and new_label == nil
        and new_text_color == nil
    then
        return true
    end
    if not player_exists(player) then return false end
    ...
    if hb.settings.bar_type == 'progress_bar' then
        ...
        if new_bar ~= nil then player:hud_change(hudtable.hudids[name].bar, 'text', new_bar) end
        if new_label ~= nil then
            hudtable.label = new_label
            local new_text = make_label(
                hudtable.format_string,
                hudtable.format_string_config,
                new_label,
                hudtable.hudstate[name].value,
                hudtable.hudstate[name].max
            )
            player:hud_change(hudtable.hudids[name].text, 'text', new_text)
        end
        if new_text_color ~= nil then player:hud_change(hudtable.hudids[name].text, 'number', new_text_color) end
    else
    ...
    local main_error_text = '[hudbars] Bad call to hb.change_hudbar, identifier: “'
        .. tostring(identifier)
        .. '”, player name: “'
        .. name
        .. '”. '
...
minetest.register_on_player_hpchange(function(player)
    if hb.players[player:get_player_name()] ~= nil then update_health(player) end
end)
