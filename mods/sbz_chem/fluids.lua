...
function sbz_api.register_fluid_cell(name, def, source_name, cell_color)
    ...
    if cell_color then
        def.inventory_image = "fluid_cell.png^[fill:1x4:6,5:" .. cell_color
    end
    def.on_place = sbz_api.on_place_precedence(function(itemstack, user, pointed)
        if pointed.type ~= "node" then return end
        local _, pos = core.item_place_node(ItemStack(source_name), user, pointed)
        if pos then
            itemstack:take_item()
            core.sound_play({ name = 'mix_water_place', gain = 0.5 })
            ...
            if inv:room_for_item("main", empty_cell) then
                ...
                core.add_item(user:get_pos(), empty_cell)
            end
            return itemstack
        end
        core.item_place(ItemStack("sbz_resources:matter_dust"), user, pointed) -- this is to handle callbacks - idealist
    end)
...
core.register_craftitem(empty_cell, {
    ...
    on_place = function(itemstack, user, pointed)
        ...
        if fluid_cell then
            core.remove_node(pointed.under)
            itemstack:take_item()
            ...
            if inv:room_for_item("main", fluid_cell) then
                ...
                core.add_item(user:get_pos(), fluid_cell)
            end
            return itemstack
        end
        core.item_place(ItemStack("sbz_resources:matter_dust"), user, pointed)
    end
})
