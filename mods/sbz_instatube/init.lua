...
local instatube_insert_object = function(pos, _, stack, _, owner, ordering)
    ...
    for index = 1, #network.machines do
        local machine = network.machines[index]
        ...
        if can_insert then
            if not machine.is_tube then
                ...
                local entity = pipeworks.tube_inject_item(
                    ...
                    { x = machine.dir.x, y = machine.dir.y, z = machine.dir.z, speed = 4 },
                    stack,
                    owner
                )
                if machine.tube.can_go then
                    can_insert = can_insert
                        and machine.tube.can_go(mpos, mnode, { x = 0, y = 0, z = 0, speed = 1 }, stack, {})
                end
                if can_insert then
                    if machine.tube.insert_object then
                        machine.tube.insert_object(mpos, mnode, stack, { x = 0, y = 0, z = 0, speed = 1 }, owner)
                    end
...
local function instatube_can_insert(pos, node, stack, vel, owner)
    ...
    for _, machine in ipairs(network.machines) do
        ...
        if machine.tube.can_insert then
            can_insert = can_insert
                and machine.tube.can_insert(mpos, mnode, stack, { x = 0, y = 0, z = 0, speed = 1 }, owner)
        end
        if can_insert then
            local filter_logic = machine.filter_logic
        ...
        if can_insert then return true end
    end
    return false
end
...
core.register_node(
    'sbz_instatube:item_filter',
    unifieddyes.def {
        description = S("Instatube Item Filter"),
...
instatube.special_filter_logic['sbz_instatube:item_filter'] = function(pos, node, dir, stack)
    ...
    for i = 1, 5 do
        ...
        if fentry[1] == stack_name and fentry[2] <= stack_count then return true end
    end
    return false
end
    ...
    iterate_around_pos(pos, function(ipos)
        ...
        if nets_at_hpos then
            for k, v in ipairs(nets_at_hpos) do
                if instatube.networks[v] then instatube.networks[v] = nil end
            end
        end
...
instatube.special_insert_logic['sbz_instatube:teleport_instant_tube'] = function(pos)
    ...
    if #receivers == 0 then return true end
    return receivers
end
...
core.register_on_mods_loaded(function()
    for name, def in pairs(core.registered_nodes) do
        if core.get_item_group(name, 'instatube') > 0 or core.get_item_group(name, 'tubedevice') > 0 then
            ...
            if og_on_rotate == false or og_on_rotate == nil then new_on_rotate = false end
    ...
    for i in A:iterp(p1, p2) do
        ...
        if p2n then
            for k, v in pairs(p2n) do
                if v == net then core.add_entity(p, 'sbz_base:debug_entity') end
            end
        end
    end
...
instatube.get_machine = function(pos, net_id)
    ...
    for k, v in pairs(net.machines) do
        if vector.equals(v.pos, pos) then core.debug(dump(v)) end
    end
end
...
button_exit[0,10;10,1;exit;Exit]
    ...
    for k, v in ipairs(nets_at_pos) do
        if instatube.networks[v] then
            ...
            if v == chosen_net_id then dropdown_id = count end
        end
    end
    ...
    on_use = function(stack, user, pointed)
        ...
        if not chosen_net then
            ...
            for k, v in pairs(nets) do
                if instatube.networks[v] then table.insert(real_nets, v) end
            end
