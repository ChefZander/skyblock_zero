		...
		func = function(name, param)
			if not param or param == "" then
				...
				if #params_list >= minarg and #params_list <= maxarg then
					...
					core.chat_send_player(name, S("Invalid command parameters: %s", table.concat(params_list," ")))
				end
			end
		end
	})
...
local metatool_privileged_give = function(name, params)
		...
		local count = 1
		if not tooldef then
			core.chat_send_player(name, S("Tool not available: %s", toolname))
			return
		end
		if #params > 1 then
			count = tonumber(params[2])
			if not count or math.floor(count) ~= count or count < 1 or count > 99 then
				core.chat_send_player(name, S("Tool count must be between %d and %d", 1, 99))
				return
			end
		end
		local player = core.get_player_by_name(name)
		if not core.check_player_privs(player, tooldef.privs) then
			core.chat_send_player(name, S("Tool not available: %s", toolname))
			return
		end
		...
		if inv:room_for_item("main", stack) then
			...
			core.chat_send_player(name, S("%d %s added to inventory", count, tooldef.itemname))
			return true
		else
			core.chat_send_player(name, S("Not enough inventory space for %s", tooldef.itemname))
			return true
		end
end
...
local metatool_privileged_list = function(name)
	...
	core.chat_send_player(name, S("Available tools: %s", table.concat(available_tools, ", ")))
	return true
end

metatool.chat.register_command_give = function()
	if not core.registered_chatcommands["metatool:give"] then
