...
local function set_wielder_formspec(def, meta)
    ...
    local fs = 'formspec_version[2]size['
        .. size
        .. ']'
        .. pipeworks.fs_helpers.get_prepends(size)
        .. list_bg
        .. 'item_image[0.5,0.3;1,1;'
        .. def.name
        .. ']'
        .. 'label[1.75,0.8;'
        ...
        .. height
        .. ';]'
        .. pipeworks.fs_helpers.get_inv(inv_offset)
        .. 'listring[]'
...
local function wielder_action(def, pos, node, index)
    ...
    local wield_index
    if index then
        if list[index] and (def.wield_hand or not list[index]:is_empty()) then wield_index = index end
    else
        ...
        after_dig_node = function(pos, oldnode, oldmetadata, digger)
            ...
            for _, stack in ipairs(oldmetadata.inventory[def.wield_inv.name] or {}) do
                if not stack:is_empty() then
                    ...
                    if leftover and not leftover:is_empty() then core.item_drop(leftover, nil, pos) end
                end
            end
            pipeworks.scan_for_tube_objects(pos)
        end,
    ...
    action = function(fakeplayer, pointed)
        ...
        if stack:get_name() == old_stack:get_name() then
            ...
            if
                ...
                and core.get_item_group(stack:get_name(), 'disable_repair') ~= 1
            then
...
pipeworks.register_wielder {
    ...
    action = function(fakeplayer, pointed)
        ...
        if not node_def.on_punch then return false end
