...
pipeworks.register_tube('pipeworks:accelerator_tube', {
    ...
    noctr = {
        { name = 'basic_tube_noctr.png', backface_culling = pipeworks.tube_backface_culling, color = 'springgreen' },
    },
...
pipeworks.register_tube('pipeworks:one_direction_tube', {
    ...
    node_def = {
        ...
        sbz_on_hover = function(pointed_thing, player)
            ...
            core.add_particle {
                pos = vector.add(pointed_thing.under, vector.divide(dir, 2)),
                expirationtime = 1,
                size = 4,
                texture = 'star.png',
        ...
        on_punch = function(pos, node, puncher, pointed_thing)
            if
                puncher
                and not puncher.is_fake_player
                and puncher:is_player()
                and not core.is_protected(pos, puncher:get_player_name())
            then
...
pipeworks.register_tube('pipeworks:low_priority_tube', {
    ...
    noctr = {
        { name = 'basic_tube_noctr.png', backface_culling = pipeworks.tube_backface_culling, color = 'lightgreen' },
    },
    node_def = {
        tube = {
            priority = 1,
