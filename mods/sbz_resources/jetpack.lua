...
minetest.register_tool('sbz_resources:jetpack', {
    ...
    on_use = function(itemstack, user, pointed_thing)
        ...
        if not user or user.is_fake_player then return itemstack end
    ...
    for player in pairs(jetpack_users) do
        ...
        if real_player and real_player:is_valid() then
            ...
            elseif controls.jump and jetpack_users[player] then
                ...
                jetpack_item:set_wear(
                    math.min(65535, jetpack_item:get_wear() + (65535 * (jetpack_durability_s ^ -1)) * dtime)
                ) -- this works, do not question it
